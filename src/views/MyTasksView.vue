...
<template>
  <div class="p-6">
    <div class="max-w-6xl mx-auto">
      <!-- Header -->
      <div class="mb-6">
        <h1 class="text-2xl font-bold text-content">{{ t('nav.myTasks') }}</h1>
        <p class="text-content-secondary mt-1">{{ t('task.assignedToMe') }}</p>
      </div>
      ...
      <div class="grid grid-cols-3 gap-4 mb-6">
        <div class="bg-surface rounded-lg border border-edge p-4">
          ...
          <div class="text-sm text-content-secondary">{{ t('task.totalTasks') }}</div>
        </div>
        <div class="bg-surface rounded-lg border border-edge p-4">
          <div class="text-2xl font-bold text-error">{{ tasks.filter(isOverdue).length }}</div>
          <div class="text-sm text-content-secondary">{{ t('task.overdueTasks') }}</div>
        </div>
        <div class="bg-surface rounded-lg border border-edge p-4">
          <div class="text-2xl font-bold text-warning">{{ tasks.filter(isDueSoon).length }}</div>
          <div class="text-sm text-content-secondary">{{ t('task.dueSoon') }}</div>
        </div>
      </div>
      ...
      <div v-else class="bg-surface rounded-lg border border-edge overflow-hidden">
            ...
            >
              ...
              <td class="px-4 py-3">
                ...
                >
                  ...
                  <span v-if="isOverdue(task)" class="ml-1">({{ t('task.overdue') }})</span>
                  <span v-else-if="isDueSoon(task)" class="ml-1">({{ t('task.dueSoon') }})</span>
                </span>
              </td>
              <td class="px-4 py-3">
        ...
        <div v-if="sortedTasks.length === 0" class="p-12 text-center text-content-secondary">
          ...
          <p class="text-sm text-content-tertiary mt-1">{{ t('task.allTasksCompleted') }}</p>
        </div>
      </div>
    </div>
