...
// Sorting
const sortBy = ref<'date_creation' | 'date_due' | 'priority'>('date_creation')
...
const loadTasks = async () => {
  ...
  try {
    ...
    for (const project of projectsStore.projects) {
      try {
        const result = await tasksStore.searchTasks('status:open', project.id)
        if (result && result.length > 0) {
          allProjectTasks.push(...result)
        }
      } catch {
...
const sortedTasks = computed(() => {
  ...
  sorted.sort((a, b) => {
    ...
    if (sortBy.value === 'date_creation') {
      ...
      comparison = (a.date_due || 0) - (b.date_due || 0)
    ...
    <div class="max-w-6xl mx-auto">
      ...
      <div v-else class="bg-surface rounded-lg border border-edge overflow-hidden">
            ...
            >
              ...
              <td class="px-4 py-3">
                <span
                  :class="[
                    'text-sm',
                    isOverdue(task) ? 'text-error font-medium' : 'text-content-secondary'
                  ]"
                >
        ...
        <div v-if="sortedTasks.length === 0" class="p-12 text-center text-content-secondary">
          <font-awesome-icon icon="clipboard-list" class="w-16 h-16 mx-auto mb-4 text-content-tertiary" />
          <p>沒有開啟中的任務</p>
        </div>
      </div>
    </div>
